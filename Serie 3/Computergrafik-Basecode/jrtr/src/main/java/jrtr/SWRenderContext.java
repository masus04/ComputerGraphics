...
public class SWRenderContext implements RenderContext {
	...
	private static Matrix4f c, p, m, d;
	...
	private ArrayList<Triangle> extractTriangles(RenderItem renderItem) {
		...
		for (int i = 0; i < indices.length; i += 9) {
			p1 = new Vector4f(positions[indices[i + 0]], positions[indices[i + 1]], positions[indices[i + 2]], 1);
			p2 = new Vector4f(positions[indices[i + 3]], positions[indices[i + 4]], positions[indices[i + 5]], 1);
			p3 = new Vector4f(positions[indices[i + 6]], positions[indices[i + 7]], positions[indices[i + 8]], 1);
