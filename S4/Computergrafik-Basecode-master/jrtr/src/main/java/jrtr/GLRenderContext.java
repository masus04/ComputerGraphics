...
//						System.out.print("Could not get location of uniform variable " + lightString + "\n");
					...
					// Pass diffuse light color to shader, we assume the shader stores it in an array "lightColorDiffuse[]"
					lightString = "lightColorDiffuse[" + nLights + "]";			
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.diffuse.x, l.diffuse.y, l.diffuse.z);		// Set diffuse light color
					else
						System.out.print("Could not get location of uniform variable " + lightString + "\n");
					
					// Pass specular light color to shader, we assume the shader stores it in an array "lightColorSpecular[]"
					lightString = "lightColorSpecular[" + nLights + "]";			
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.specular.x, l.specular.y, l.specular.z);	// Set specular light color
					else
						System.out.print("Could not get location of uniform variable " + lightString + "\n");
					
					// Pass ambient light color to shader, we assume the shader stores it in an array "lightColorAmbient[]"
					lightString = "lightColorAmbient[" + nLights + "]";			
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.ambient.x, l.ambient.y, l.ambient.z);	// Set ambient light color
					else
						System.out.print("Could not get location of uniform variable " + lightString + "\n");
				...
				id = gl.glGetUniformLocation(activeShaderID, "centerOfProjection");
				if (id != -1)
					gl.glUniform3f(id, camera.getCenterOfProjection().x, camera.getCenterOfProjection().y, camera.getCenterOfProjection().z);
				else
					System.out.print("Could not get location of uniform variable " + "centerOfProjection" + "\n");
				 
				String materialString;
				// Pass diffuse material color to shader, we assume the shader stores it in "materialColorDiffuse"
				materialString = "materialColorDiffuse";
				id = gl.glGetUniformLocation(activeShaderID, materialString);
				if(id!=-1)
					gl.glUniform3f(id, m.diffuse.x, m.diffuse.y, m.diffuse.z);		// Set diffuse material color
				else
					System.out.print("Could not get location of uniform variable " + materialString + "\n");
				
				// Pass specular material color to shader, we assume the shader stores it in "materialColorSpecular"
				materialString = "materialColorSpecular";
				id = gl.glGetUniformLocation(activeShaderID, materialString);
				if(id!=-1)
					gl.glUniform3f(id, m.specular.x, m.specular.y, m.specular.z);		// Set specular material color
				else
					System.out.print("Could not get location of uniform variable " + materialString + "\n");
				
				// Pass ambient material color to shader, we assume the shader stores it in "materialColorAmbient"
				materialString = "materialColorAmbient";
				id = gl.glGetUniformLocation(activeShaderID, materialString);
				if(id!=-1)
					gl.glUniform3f(id, m.ambient.x, m.ambient.y, m.ambient.z);		// Set ambient material color
				else
					System.out.print("Could not get location of uniform variable " + materialString + "\n");
				
				// Pass material shininess to shader, we assume the shader stores it in "materialShininess"
				materialString = "materialShininess";
				id = gl.glGetUniformLocation(activeShaderID, materialString);
				if(id!=-1)
					gl.glUniform1f(id, m.shininess);		// Set material shininess
				else
					System.out.print("Could not get location of uniform variable " + materialString + "\n");
