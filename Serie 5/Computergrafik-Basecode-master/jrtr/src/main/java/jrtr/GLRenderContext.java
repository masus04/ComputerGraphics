...
public class GLRenderContext implements RenderContext {
	...
	private void draw(RenderItem renderItem) {		
		...
		// shader may have changed since the vertex array object was
		// initialized.
	 ...
	 * (VAO) for efficient rendering. The VAO stores several
	 * "vertex buffer objects" (VBOs) that contain the vertex attribute data.
	 * 
	 * @param data
	...
	private void setMaterial(Material m) {
		...
		if (m != null && m.shader != null) {
			...
			if (m.texture != null) {
				...
				gl.glUniform1i(id, 0); // The variable in the shader needs to be
										// set to the desired texture unit,
										// i.e., 0
			}
			...
			if (iter != null) {
				...
				// Pass number of lights to shader, we assume this is in a
				// variable "nLights" in the shader
